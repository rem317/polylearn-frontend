...
const AppState = {
    ...
    selectedApp: 'mathease', // Default to polylearn
    previousPage: null,
...
async function fetchAllLessons(appName = null) {
    try {
        ...
        // Kung wala sa localStorage, saka lang gumamit ng default na 'mathease'
        const selectedApp = appName || localStorage.getItem('selectedApp') || 'mathease';
...
async function updateContinueLearningModule() {
    try {
        ...
        // Kung wala, gumamit ng 'mathease' bilang default
...
async function openLesson(lessonId) {
    try {
        ...
        if (lesson) {
            ...
            LessonState.currentTopic = lesson.topic_id || 4;
...
async function loadPracticeExercises(topicId) {
    try {
        ...
        if (!response.ok) {
            console.error(`❌ Failed to load practice exercises: ${response.status} ${response.statusText}`);
            return null;
        }
        ...
        showNotification('Failed to load practice exercises', 'error');
        return null;
    }
}
...
async function loadPracticeExercisesForTopic(topicId) {
    try {
        ...
        if (practiceData.exercises) {
            practiceData.exercises.forEach((exercise, index) => {
                console.log(`Exercise ${index + 1}:`, {
                    id: exercise.exercise_id,
                    title: exercise.title,
                    hasContentJson: !!exercise.content_json,
                    typeOfContentJson: typeof exercise.content_json,
                    contentJsonPreview: exercise.content_json ? JSON.stringify(exercise.content_json).substring(0, 100) + '...' : 'none'
                });
            });
        }
        
        if (!practiceData.unlocked) {
            exerciseArea.innerHTML = createPracticeLockScreen(practiceData);
            return;
        }
